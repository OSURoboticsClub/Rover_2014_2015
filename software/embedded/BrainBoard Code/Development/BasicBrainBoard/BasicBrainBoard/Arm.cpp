...
void home_all(){
	...
	while(pressed < 5){
		...
		for(int i=0;i<7;i++){
			if(limit_pressed(ARM_Z))pressed++;
		}
	}
	stop_axis(ARM_Z);
	set_zero(ARM_Z);
...
void armMain(){
	...
	while(1){
		...
		if(homed){
			int16_t z_top = -8250; /* Step position where z is straight up. */
			//if(armData.xAxisValue > 128){ /* If arm target is to the left */
				//if(ArmAxis[ARM_X].current < (int32_t) (ArmAxis[ARM_X].steps_per * 128)){ /* If arm is currently on the right. */
				...
				//wait_until_stopped();
			//} else {
				//if(ArmAxis[ARM_X].current > (int32_t) ArmAxis[ARM_X].steps_per * 128){ /* If arm is currently on the left. */
					//set_target(ARM_Z, z_top);
					//wait_until_stopped();
					//set_target(ARM_X, ArmAxis[ARM_X].steps_per * armData.xAxisValue);
				//}
