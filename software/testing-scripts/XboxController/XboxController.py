...
def serialParser(lThumb, rThumb):
  print "\n\n## Running Serial Parser ##"
  
  lSpeed = int( clamp(float(lThumb) * (2.55 / 2) + 127, 0, 255) )
  rSpeed = int( clamp(float(rThumb) * (2.55 / 2) + 127, 0, 255) )

  print "Left Speed: " + str(lSpeed)
  print "Right Speed: " + str(rSpeed)

  packet_to_send = [255, lSpeed, rSpeed, 0, 0, 0, 0, 255]
 
  print "## End of Serial Parser ##\n\n" 
...
#time.sleep(5)
        ...
        time.sleep(5)
        print "Poking microcontroller..."
        s.write('p')
        device_id = s.readline()
        print(device_id)
        if device_id.strip() == "DRIVE".strip():
          print("## Matched ID ##")
          s.write('r')
          serialPort = s
        #  except (OSError, serial.SerialException):
        #    pass
#ser = serial.Serial(0)i
        ''' 
        if (not('serialPort' in locals() or 'serialPort' in globals())):
          print "#!!# Critical Error: No serial device found #!!#"
          while 1:
            pass
        else:
          print "# Found a drive board! #"
        '''
...
#time.sleep(5)
        ...
        while True:
          time.sleep(1)
          packet = serialParser(xboxCont.LTHUMBY, xboxCont.RTHUMBY)
          print "Packet to send: " + str(packet)
	  print len(packet)
          for x in range(len(packet)):
            serialPort.write(chr(packet[x]))
            print "wrote stuffz: " + chr(   packet[x]  )
	  print serialPort.readline()
#          finalPacket_struct = 'cccccccc'
#	  finalPacket = struct.pack(finalPacket_struct, chr(0xFF), chr(packet[1]), chr(packet[2])
#          for x in range(len(packet)):
#            finalPacket[x] = chr(packet[x])
#	  print finalPacket
